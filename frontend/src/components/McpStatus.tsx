...
export default function McpStatus({ url, onUrlChange, conn, connecting, error, onReconnect }: McpStatusProps) {
  ...
  return (
    <div className="space-y-2">
      <div>
        <label className="text-xs font-semibold text-slate-400 block mb-1.5">URL do servidor</label>
        <input
          type="text"
          value={url}
          onChange={(e) => onUrlChange(e.target.value)}
          className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs font-mono text-slate-300 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
        />
      </div>

      <div className="flex items-center justify-between bg-slate-800 border border-slate-700/50 rounded-xl px-3 py-2">
        ...
        <button
          onClick={onReconnect}
          disabled={connecting}
          className="text-[11px] text-blue-400 hover:text-blue-300 font-medium disabled:opacity-40 transition-colors"
        >
          reconectar
        </button>
      </div>
