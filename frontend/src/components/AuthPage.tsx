...
export default function AuthPage({ conn, connecting, connError, onAuthenticated, isDark, onToggleTheme }: AuthPageProps) {
  ...
  const mcpColors = {
    ...
    connecting: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20',
  ...
  const inputCls =
    'w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 ' +
    'placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/70 ' +
    'focus:border-blue-500/50 focus:bg-white dark:focus:bg-white/8 transition-all';

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex items-center justify-center p-4 overflow-hidden relative">

      {/* Toggle tema */}
      <button
        onClick={onToggleTheme}
        title={isDark ? 'Mudar para modo claro' : 'Mudar para modo escuro'}
        className="absolute top-4 right-4 z-20 w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-white/8 border border-slate-200 dark:border-white/10 transition-all bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm"
      >
        {isDark ? <SunIcon /> : <MoonIcon />}
      </button>

      {/* Orbs de fundo animados */}
      <div className="absolute top-[-180px] left-[-80px] w-[550px] h-[550px] bg-blue-500/10 dark:bg-blue-600/18 rounded-full blur-[110px] animate-orb-1 pointer-events-none" />
      <div className="absolute bottom-[-150px] right-[-60px] w-[480px] h-[480px] bg-violet-500/8 dark:bg-violet-600/14 rounded-full blur-[100px] animate-orb-2 pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-blue-500/8 dark:bg-blue-900/20 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-sm z-10">
        ...
        {/* Card */}
        <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-white/8 rounded-2xl p-7 shadow-xl dark:shadow-2xl space-y-5">
          ...
          <div className="flex bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/8 rounded-xl p-1">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-slate-100 hover:border-slate-300 dark:hover:border-white/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-600">ou</span>
            <div className="flex-1 h-px bg-slate-200 dark:bg-white/6" />
          </div>
          ...
          <form onSubmit={submitEmailFlow} className="space-y-3.5">
            {mode === 'register' && (
              <div>
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 block mb-1.5">Nome completo</label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 block mb-1.5">E-mail</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder="seu@email.com" autoComplete="email" className={inputCls} />
            </div>

            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 block mb-1.5">Senha</label>
        ...
        <p className="text-[11px] text-slate-400 dark:text-slate-700 text-center mt-5 px-2">
