...
export default function ToolCallCard({ call }: ToolCallCardProps) {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden text-xs transition-all ${containerCls}`}>
      ...
      >
        {isRunning && (
          <svg className="w-3.5 h-3.5 text-blue-400 animate-spin-slow flex-shrink-0" viewBox="0 0 24 24" fill="none">
        ...
        {isError && (
          <svg className="w-3.5 h-3.5 text-red-400 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
        ...
        <code className="font-mono font-semibold text-slate-300 text-[11px]">{call.name}</code>

        {argsStr && (
          <span className="text-[11px] text-slate-600 truncate font-mono flex-1 min-w-0">
            {argsStr}
          </span>
        )}

        {call.durationMs !== undefined && (
          <span className="text-[10px] text-slate-600 ml-auto flex-shrink-0 tabular-nums">
      ...
      {expanded && (
        <div className="border-t border-white/5 bg-black/20 px-3 py-3 space-y-3">
          {Object.keys(safeArgs).length > 0 && (
            <div>
              <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-wider mb-1.5">Argumentos</p>
              <pre className="text-[11px] font-mono bg-slate-900 text-slate-300 border border-slate-700 rounded-lg p-2.5 overflow-x-auto">
